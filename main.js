...
(function initMockupVideo() {
  ...
  video.addEventListener('timeupdate', () => {
    ...
    if (video.currentTime > video.duration - 0.8) { fadingOut = true; video.style.opacity = '0'; }
  });
...
(function initSpaceBackground() {
  ...
  if (typeof Lenis !== 'undefined') {
    ...
    function lenisLoop(ts) {
      lenis.raf(ts);
      curFarX  += (tgtFarX  - curFarX)  * LERP; curFarY  += (tgtFarY  - curFarY)  * LERP;
      curMidX  += (tgtMidX  - curMidX)  * LERP; curMidY  += (tgtMidY  - curMidY)  * LERP;
      curNearX += (tgtNearX - curNearX) * LERP; curNearY += (tgtNearY - curNearY) * LERP;
      applyTransforms();
...
(function initComoFunciona() {
  ...
  if (cfTitle) {
    ScrollTrigger.create({
      ...
      onEnter: function() { gsap.from(cfTitle, { opacity: 0, y: 30, duration: 0.7, ease: 'power2.out' }); },
      once: true
    });
  }
  if (cfHeadline && typeof SplitType !== 'undefined') {
    ...
    if (split.chars && split.chars.length) {
      ...
      ScrollTrigger.create({
        trigger: cfHeadline, start: 'top 85%',
        onEnter: function() { gsap.to(split.chars, { opacity: 1, y: 0, stagger: 0.04, duration: 0.7, ease: 'power2.out' }); },
        once: true
      });
    }
  }
  if (isMobile) {
    cfCards.forEach(function(card) {
      ...
      ScrollTrigger.create({
        ...
        onEnter: function() { gsap.to(card, { opacity: 1, y: 0, rotation: deg, scale: 1, duration: 0.8, ease: 'power2.out' }); },
        once: true
      });
    });
    cfLines.forEach(function(line) {
      ...
      ScrollTrigger.create({
        ...
        onEnter: function() { gsap.to(line, { scaleY: 1, duration: 0.6, ease: 'power2.out' }); },
        once: true
      });
    });
  } else {
    ...
    function showStep(i) {
      ...
      gsap.to([stepBig, stepLabel], { opacity: 0, duration: 0.2, onComplete: function() {
        stepBig.textContent = stepData[i].num; stepLabel.textContent = stepData[i].name;
  ...
  if (hlC && typeof SplitType !== 'undefined') {
    ...
    if (split.chars && split.chars.length) {
      ...
      ScrollTrigger.create({
        ...
        start: 'top 85%',
        onEnter: function() {
          gsap.to(split.chars, { opacity: 1, y: 0, stagger: 0.05, duration: 0.6, ease: 'power2.out' });
        },
        once: true
      });
    }
